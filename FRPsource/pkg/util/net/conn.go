...
func ConnectServerByProxy(proxyURL string, protocol string, addr string, websocketDomain string) (c net.Conn, err error) {
	switch protocol {
	case "tcp":
	...
	case "websocket":
		return ConnectWebsocketServer(addr, websocketDomain, false)
	case "wss":
		return ConnectWebsocketServer(addr, websocketDomain, true)
	default:
		return nil, fmt.Errorf("unsupport protocol: %s", protocol)
	}
}

func ConnectServerByProxyWithTLS(proxyURL string, protocol string, addr string, tlsConfig *tls.Config, websocketDomain string) (c net.Conn, err error) {
	c, err = ConnectServerByProxy(proxyURL, protocol, addr, websocketDomain)
	if err != nil {
		return
	}
